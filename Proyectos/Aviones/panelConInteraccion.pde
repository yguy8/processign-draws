...
void draw() {
  ...
  // Primera fila
  dibujarVelocimetro(180, 130, "SPD", velocidad);
  dibujarBrújula(400, 130, "HDG", rumbo);
  dibujarAltimetro(620, 130, "ALT", altitud);

  // Segunda fila
  dibujarHorizonte(180, 270);
  dibujarVerticalSpeed(400, 270, "VSI", verticalSpeed);
  dibujarTurnCoordinator(620, 270);

  // Tercera fila
  dibujarFuelGauge(180, 410, "FUEL", fuel);
  dibujarDisplay(400, 410, "1200");
  dibujarEngineTemp(620, 410, "ENG", engineTemp);
...
void keyPressed() {
  ...
  if (key == 'a') rumbo -= 1;
  if (key == 'd') rumbo += 1;
  if (key == 'q') velocidad += 5;
  if (key == 'e') velocidad -= 5;
  if (key == 'z') verticalSpeed += 1;
  if (key == 'x') verticalSpeed -= 1;
  if (key == 'f') fuel -= 0.05;
  if (key == 't') engineTemp += 0.05;

  // Limitar valores
  velocidad = constrain(velocidad, 0, 160);
  altitud = constrain(altitud, 0, 100);
  rumbo = constrain(rumbo, 0, 12); // 12 pasos de 30° = 360°
  verticalSpeed = constrain(verticalSpeed, -20, 20);
  fuel = constrain(fuel, 0, 1);
  engineTemp = constrain(engineTemp, 0, 1);
}
...
void dibujarBrújula(float x, float y, String label, float rumbo) {
  dibujarDial(x, y, label, 0, 12, rumbo, color(255));
}
